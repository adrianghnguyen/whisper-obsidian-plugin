...
```text
# T014 (WhisperSettingsTab.ts) and T015 (verify T013 path) can run in parallel
```

## Parallel Example: User Story 3

```text
# Sequential: T016 (first-run progress in src/localAsr/index.ts and/or src/StatusBar.ts) then T017 (cache reuse in src/localAsr/index.ts)
```
