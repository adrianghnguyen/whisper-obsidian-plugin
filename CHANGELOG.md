...
* **Per-provider enable toggles.** Hide unused transcription providers from the active dropdown and status-bar cycle without removing saved config. The last enabled provider cannot be turned off; if the active provider is disabled, Whisper falls back to another enabled one.
* **Status bar soft red pulse while recording.** The recording indicator pulses softly in red so the live session is easier to spot at a glance.

### Bug Fixes

* **Debounce textarea saves in Settings** so editing long prompts no longer causes UI jank from per-keystroke disk writes.
* **Remove unused Gemini Live system instruction setting.** That control no longer appears in Advanced settings.
...
* recording controls UX ΓÇö show/hide buttons, paused state ([0fbd64c](https://github.com/nikdanilov/whisper-obsidian-plugin/commit/0fbd64c368a3cd13e7319179aee108ffc1589963))
